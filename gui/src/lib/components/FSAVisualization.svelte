<script lang="ts">
    ...
    function hasTransitionBeenVisited(transition: any): boolean {
    ...
    $: transitionData = possibleTransitions.map(transition => ({
        ...transition,
        visited: hasTransitionBeenVisited(transition),
        ...
        shouldShow: dynamicStates[transition.from] && dynamicStates[transition.to] && hasTransitionBeenVisited(transition)
    }));
...
<div class="fsa-visualization">
    ...
    <div class="panel-content">
        ...
        <svg class="fsa-graph" viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg">
            ...
            {#each transitionData as { visited, arrowPos, path, shouldShow, label, color }, index (index)}
                {#if shouldShow}
                    <g class="transition">
                        <path
                            d={path}
