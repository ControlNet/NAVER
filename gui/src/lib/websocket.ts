...
class WebSocketService {
    ...
    connect(newSessionId: string | null = null): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            let baseUrl = PUBLIC_BASE_URL === '' ? window.location.host : PUBLIC_BASE_URL;
            const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            if (baseUrl.startsWith('http')) {
                baseUrl = baseUrl.split('://')[1];
                baseUrl = baseUrl.replace(/\/$/, '');
            }
            const wsUrl = `${protocol}//${baseUrl}/ws/${this.currentSessionId}`;
    ...
    async startAnalysis(imageFile: File, query: string): Promise<AnalysisResponse> {
        ...
        const htmlProtocol = window.location.protocol;
        let baseUrl = PUBLIC_BASE_URL === '' ? window.location.host : PUBLIC_BASE_URL;
        ...
        try {
            ...
            const response = await fetch(`${htmlProtocol}//${baseUrl}/api/start`, {
                method: 'POST',
                body: formData
            });
